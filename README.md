...
**LifeReminder - 时光提醒** 是一款基于 **Cloudflare Workers** 的全栈生活事件提醒工具。专为管理**生日、纪念日、节日、周期性日程**等场景设计。无需服务器，零成本托管，提供精美的机甲风 UI 界面、强大的农历/公历计算核心、多渠道通知推送能力以及 iCal 日程同步。

> 🎯 **核心场景**：再也不会忘记家人生日、结婚纪念日、朋友生日、传统节日！

<div align="center">
  <img src="./assets/mainUI_lightCN_shot.png" alt="LifeReminder 界面预览" width="800">
  <img src="./assets/mainUI_darkEN_shot.png" alt="LifeReminder 界面预览" width="800">
</div>
...
- **📅 智能周期管理**：
  - 支持**公历**与**农历**（Lunar）周期计算。内置高精度农历算法（1900-2100）。
  - 农历生日、传统节日自动转换计算，永远准确提醒！
  - 支持按天、月、年为周期的自动推算。
  - 提供"周期事件"（每年循环）与"单次事件"（一次性倒计时）两种模式。
- **🎉 内置节日库**：
  - 一键添加常见节日：春节、元宵节、清明节、端午节、中秋节、国庆节...
  - 西方节日：情人节、母亲节、父亲节、圣诞节...
  - 自动识别农历/公历，开箱即用！
- **🔔 多渠道通知**：
  - 内置支持 **Telegram, Bark, PushPlus, NotifyX, Resend (Email), 飞书, 企业微信, Webhook**。
  - 支持自定义提前提醒天数和每日推送时间。
  - **周年数显示**：生日通知自动显示"第 X 年"，纪念日自动计算"结婚 X 周年"！
- **🤖 自动化管理**：
  - **自动周期**：事件到来后自动更新到下一周期。
  - **自动禁用**：过期太久未处理的事件自动标记为禁用。
  - **Cron 触发**：支持通过 Cloudflare Cron Triggers 每日定时检查。
- **📆 ICS 日历订阅**：提供标准的 `.ics` 订阅链接，可完美接入 iOS 日历、Google Calendar 或 Outlook。
- **🛡️ 安全可靠**：
...
[![Deploy to Cloudflare Workers](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/ieax/lifereminder)
...
3.  给 Worker 起个名字，例如 `lifereminder`，点击 **Deploy**。
...
部署成功后，访问您的 Worker 域名或您添加的自定义域名（如 `https://lifereminder.your-name.workers.dev`或 `https://lifereminder.your-domain.com`）。
...
<div align="center">
  <img src="./assets/configUIhelp_darkCN_shot.png" alt="LifeReminder 界面预览" width="800">
</div>
...
| **飞书**          | **Webhook URL**: 飞书机器人地址                                     | 1. 在飞书群聊中添加"自定义机器人"。<br>2. 复制 Webhook 地址。                                                                                                                                                                                |
| **企业微信**      | **Webhook URL**: 企业微信机器人地址                                 | 1. 在企业微信群聊中添加"群机器人"。<br>2. 复制 Webhook 地址。                                                                                                                                                                                |
...
点击"**新增事件**"按钮的下拉箭头，可以一键添加常见节日：

- 🧧 **中国传统节日**：春节、元宵节、清明节、端午节、七夕、中秋节、重阳节
- 🎄 **西方节日**：情人节、母亲节、父亲节、万圣节、感恩节、圣诞节
- 🎊 **公共假日**：元旦、妇女节、劳动节、儿童节、国庆节

系统会自动设置正确的农历/公历模式，开箱即用！

### 添加自定义事件

- **名称**：事件的名称（如 "爸爸生日"，"结婚纪念日"）。
- **标签**：用于分类（如 `生日`, `纪念日`, `节日`, `日程`），支持多选。
- **模式**：
  - 🔄 **周期事件**：每年循环的事件，如生日、纪念日、节日。
  - ⏳ **单次事件**：一次性倒计时事件，如考试日期、旅行出发日。
- **农历开关**：开启后，周期将按农历计算（适合农历生日）。
- **自动化策略**：
  - **自动周期**：事件到来后自动将下次日期顺延一个周期。
  - **自动禁用**：过期超过指定天数未处理，自动标记为禁用。

<div align="center">
  <img src="./assets/AddUI_darkCN_shot.png" alt="LifeReminder 界面预览" width="600">
</div>
...
```
【⏳ 事件即将到来】
1. 爸爸生日: 还剩 3 天 (2025-12-28)（52 年）
2. 结婚纪念日: 就是今天 (2025-12-25)（10 年）
3. 春节: 还剩 30 天 (2026-01-29)
```

对于年周期的事件（如生日、纪念日），系统会自动计算并显示是第几个周年！
...
5.  **手机自带日程 APP**: 在 CalDAV 服务商添加订阅链接，手机添加 CalDAV 订阅信息进行日程同步。
...
如果您觉得 LifeReminder 对您有帮助，欢迎请作者喝一杯咖啡 ☕️。您的支持是我持续更新的动力！
